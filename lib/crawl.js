...
module.exports = function crawl(url, baseDir){
	function createDataFile(record){
		getLayout(record.layoutFileHref, function(err, layout){
			...
			winston.debug('Started writing %s', dataFilePath);
			...
			winston.debug('Started writing %s', layoutFilePath);
			fs.writeFile(layoutFilePath, JSON.stringify(layout, null, 2), function(){
